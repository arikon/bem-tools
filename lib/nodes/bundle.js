...
var BundleNode = exports.BundleNode = INHERIT(MagicNode, {
    ...
    __constructor: function(level, bundleNameOrItem, subBundleName) {
        ...
        this.item = (typeof bundleNameOrItem === 'string')? createItem(bundleName, subBundleName) : bundleNameOrItem;
...
exports.createBundleNode = function(level, bundleName, subBundleName) {
    var nodeClass = BundleNode,
        item = createItem(bundleName, subBundleName),
        // TODO: use tech
        makePath = level.getByObj(item) + '.make.js';

    if (PATH.existsSync(makePath)) nodeClass = INHERIT(nodeClass, require(makePath));

    return new nodeClass(level, item);
};
